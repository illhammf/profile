## 🔥 Repo Stats (Auto Update)
![Views](https://komarev.com/ghpvc/?username=illhammf&repo=crkosongdua-2026&color=blue&style=for-the-badge)

---

Jadi ini adalah tugas Pertemuan ke-4 mata kuliah Pemrograman Web
### Silahkan ikuti dengan cermat yaa

Nyalakan
```text
- Docker
- Web Browser
```

Di Ubuntu ketik:
```bash
./start.sh profile
```
tunggu sampai selesai dan sampai masuk ke Visual Studio Code
kalo sudah di install tapi belum masuk VS Code, ketik
```bash
code .
```

---

setelah masuk VS Code, kita masukan Template yang sudah di download di https://www.tooplate.com/ 
lalu masukkan isi dari template yang sudah kalian pilih

